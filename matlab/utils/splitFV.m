function [iso, maxSet] = splitFV( f, v )
...
for currentSet = 1:numSets
    ...
    if (nnz(fSets==currentSet) > maxCount)
        maxSet = currentSet;
        maxCount = nnz(fSets==currentSet);
    end
end

maxisoLength = 0; maxisoIdx = 1;
for s = 1:length(fvOut)
    if length(fvOut(s).faces) > maxisoLength 
        maxisoLength = length(fvOut(s).faces);
        maxisoIdx = s;
    end
end

iso.faces = []; iso.vertices = [];
iso.faces = fvOut(maxisoIdx).faces;
iso.vertices = fvOut(maxisoIdx).vertices;
